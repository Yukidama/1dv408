...
class Password {
	 ...
	 * @return Password
	 */
	public static function fromCleartext($cleartext, \register\model\RegisterObserver $observer = null) {
		if (self::isOkPassword($cleartext, $observer) == true ) {
			$ret = new Password();
	 ...
	 * @return boolean         
	 */
	private static function isOkPassword($string, \register\model\RegisterObserver $observer = null) {
		
		if (strlen($string) < self::MINIMUM_PASSWORD_CHARACTERS) {
			if ($observer != null) {
				$observer->passwordIsTooShort();
			}
			return false;
		} else if (strlen($string) > self::MAXIMUM_PASSWORD_CHARACTERS) {
			if ($observer != null) {
				$observer->passwordIsTooLong();
			}
			return false;
		}
		return true;
